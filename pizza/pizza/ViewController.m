...
//设计原则：要依赖抽象不要依赖具体类；这个原则听起来很像：“针对接口编程，不针对实现编程”，但这里更强调“抽象”这个原则说明了：不能让高层组件依赖低层组件，而且不管高层或低层组件，两者都应该依赖于抽象（pizzStore高层组件，pizza 低层组件）先从顶端开始然后往下，pizzaStrore不会理会具体的pizza,而是依赖抽象的pizza
- (void)viewDidLoad {
    [super viewDidLoad];
    //第一种芝加哥fengwei的pizza;
